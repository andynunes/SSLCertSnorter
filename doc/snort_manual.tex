...
\title{SNORT\textsuperscript{\textregistered} Users Manual\\2.9.1}
...
Receieve Offload" (gro). With these features enabled, the network card performs
...
is done via setting the environment variable \emph{PCAP\_FRAMES}.
...
\item Block = packets Snort did not forward, eg due to a block rule.  "Block"
...
\hline
\hline
Passive & \texttt{snort --treat-drop-as-alert} & tap & Alert\\
\hline
Passive & \texttt{snort} & tap & Not Loaded\\
\hline
Passive & \texttt{snort --treat-drop-as-alert} & inline\_test & Alert\\
\hline
Passive & \texttt{snort} & inline\_test & Would Drop\\
\hline
Passive & \texttt{snort --treat-drop-as-alert} & inline & Alert\\
\hline
Passive & \texttt{snort} & inline & Not loaded + warning\\
\hline
Inline Test & \texttt{snort --enable-inline-test --treat-drop-as-alert} & tap & Alert\\
\hline
Inline Test & \texttt{snort --enable-inline-test} & tap & Would Drop\\
\hline
Inline Test & \texttt{snort --enable-inline-test --treat-drop-as-alert} & inline\_test & Alert\\
\hline
Inline Test & \texttt{snort --enable-inline-test} & inline\_test & Would Drop\\
\hline
Inline Test & \texttt{snort --enable-inline-test --treat-drop-as-alert} & inline & Alert\\
\hline
Inline Test  & \texttt{snort --enable-inline-test} & inline & Would Drop\\
\hline
Inline & \texttt{snort -Q --treat-drop-as-alert} & tap & Alert\\
\hline
Inline & \texttt{snort -Q} & tap & Alert\\
\hline
Inline & \texttt{snort -Q --treat-drop-as-alert} & inline\_test & Alert\\
\hline
Inline & \texttt{snort -Q} & inline\_test & Would Drop\\
\hline
Inline & \texttt{snort -Q --treat-drop-as-alert} & inline & Alert\\
\hline
Inline & \texttt{snort -Q} & inline & Drop\\
\hline
\end{tabular}
...
dynamic DAQs from that directory.  You can also preceed this option with extra
...
\hline
\texttt{config vlan\_agnostic} & Causes Snort to ignore vlan headers for the purposes of connection tracking.  This option is only valid in the base configuration when using multiple configurations, and the default is off.  \\

\hline
\texttt{config policy\_mode: tap|inline|inline\_test} & Sets the policy mode to either \texttt{passive}, \texttt{inline} or \texttt{inline\_test}. \\
...
This value should be specified in the default policy even when the HTTP inspect preprocessor 
is turned off using the \texttt{disabled} keyword. It is suggested to set this value such that 
the max gzip session calculated as follows is at least 1.
...
"Space Saturation" alert with SID 26 and GID 119.  This default value for this
...
Multiple MIME attachments/data in one packet are pipelined. When stateful inspection 
is turned on the 7bit/8bit/binary/text MIME attachments/data across multiple packets are extracted too.
...
When the memcap for decoding (\texttt{max\_mime\_mem}) is exceeded the SMTP preprocessor alert with sid 9 is
...
A value of -1 turns off the base64 decoding of MIME attachments. The value of 0
sets the decoding of base64 encoded MIME attachments to unlimited. A value other
than 0 or -1 restricts the decoding of base64 MIME attachments. A POP preprocessor
...
from -1 to 65535. A value of -1 turns off the QP decoding of MIME attachments.
The value of 0 sets the decoding of QP encoded MIME attachments to unlimited. A
value other than 0 or -1 restricts the decoding of QP MIME attachments. A POP
...
value ranges from -1 to 65535. A value of -1 turns off the extraction of these MIME
attachments. The value of 0 sets the extraction of these MIME attachments to unlimited.
A value other than 0 or -1 restricts the extraction of these MIME attachments. A POP 
preprocessor alert with sid 6 is generated (if enabled) when this extraction depth is exceeded.

Multiple MIME attachments/data in one packet are pipelined. When stateful inspection
is turned on the 7bit/8bit/binary MIME attachments/data across multiple packets are extracted too.
...
value other than 0 or -1 restricts the decoding of UU POP attachments. A POP 
...
before being added to the defragmentation module.  Default is set to -1. The 
allowed range for this option is 1514 - 65535.
...
Base normalizations enabled with "preprocessor \texttt{normalize\_tcp}" include:
...
urgent pointer:  don't adjust the urgent pointer if it is greater than payload
length.
...
This preprocessor runs before other preprossors.
\subsubsection{Configuration}
The preprocessor configuration name is \texttt{repuation}.\\
...
\hline
\texttt{129} & Stream5 preprocessor \\
...
the associated event ID).  Both file types are written in a binary format
...
sdrop to help avoid confusion between packets dropped due to load (eg lack of
...
Several preprocessors, such as Telnet, RPC, and SMTP, use decoded/normalized data for
content match by default, if \texttt{rawbytes} is not specified explicitly. Therefore, 
\texttt{rawbytes} should be specified in order to inspect raw data for those traffic.
...
1. When the traffic being detected is HTTP it sets the buffer to,
	...
	d. HTTP UTF normalized response body (when \texttt{normalize\_utf} is turned on)
	e. All of the above
2. When the traffic being detected is SMTP/POP/IMAP it sets the buffer to,
	...
	c. 7bit/8bit/binary/text MIME attachment (when \texttt{bitenc\_decode\_depth} is greater than -1)
...
to set the corsor used for detection to the beginning of the base64 decoded 
buffer if present.
...
may be specified. A rule could check for a flags value of S,CE if one wishes to
...
The stream\_reassemble option is only available when the Stream5 preprocessor is
enabled.
