...
{
    *buf = smtp_ssn->log_state->filenames;
    *len = smtp_ssn->log_state->file_logged;
...
static int SMTP_GetMailFrom(void *data, uint8_t **buf, uint32_t *len, uint32_t *type)
{
    ...
    *buf = smtp_ssn->log_state->senders;
    *len = smtp_ssn->log_state->snds_logged;
...
static int SMTP_GetRcptTo(void *data, uint8_t **buf, uint32_t *len, uint32_t *type)
{
    ...
    *buf = smtp_ssn->log_state->recipients;
    *len = smtp_ssn->log_state->rcpts_logged;
